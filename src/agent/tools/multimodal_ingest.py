...
MAX_PAGES = 500  # legacy constant; runtime limit = config.multimodal_upload_max_pages
MAX_PDF_BYTES = 200 * 1024 * 1024  # legacy constant; runtime limit = config.multimodal_upload_max_mb
...
async def ingest_one_pdf(
    ...
    *,
    ...
    page_images, sizes = await asyncio.to_thread(load_page_images, pdf_path, config.dot_ocr_dpi)
    ...
    upsert_stats = await asyncio.to_thread(_upsert_in_worker_thread)
    ...
    try:
        ...
        try:
            await asyncio.to_thread(backend.replace_document_nodes, document_id)
        except Exception as cleanup_exc:
            logger.warning("[MmIngest] Milvus rollback failed for {}: {}", document_id, cleanup_exc)
        try:
            await asyncio.to_thread(assets.delete_document, document_id)
        except Exception as cleanup_exc:
