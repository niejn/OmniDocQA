...
class Config(BaseSettings):
    ...
    report_detail_full_evidence_max_chars: int = Field(default=0)  # env: REPORT_DETAIL_FULL_EVIDENCE_MAX_CHARS
    ...
    # Generic fallback for providers without a dedicated model profile.
    embedding_safe_chars: int = Field(default=3000)  # env: EMBEDDING_SAFE_CHARS
    # auto | qwen | openai | zhipu | openrouter — use openai if DashScope (dashscope.aliyuncs.com) is blocked or unstable.
    ...
    ask_evidence_llm_chars_per_source: int = Field(default=7000, ge=800, le=50000)  # env: ASK_EVIDENCE_LLM_CHARS_PER_SOURCE
    ...
    ask_evidence_llm_max_quote_chars: int = Field(default=1200, ge=200, le=8000)  # env: ASK_EVIDENCE_LLM_MAX_QUOTE_CHARS
    ...
    sec_http_user_agent: str = Field(default="rag-edgar-sync/1.0 (replace-with-your-email@example.com)")  # env: SEC_HTTP_USER_AGENT
    ...
    mixed_narrative_evidence_order: Literal["sql_before_rag", "rag_before_sql"] = Field(default="sql_before_rag")  # env: MIXED_NARRATIVE_EVIDENCE_ORDER
    ...
    mixed_narrative_sql_align_to_rag_filings: bool = Field(default=True)  # env: MIXED_NARRATIVE_SQL_ALIGN_TO_RAG_FILINGS
    # Narrative retrieval: bypass section-role hard rejection and treat section policy as soft signal.
    narrative_disable_section_hard_filter: bool = Field(default=False)  # env: NARRATIVE_DISABLE_SECTION_HARD_FILTER
    # Narrative retrieval: do not filter or score by section_role (field still stored on nodes / in traces).
    narrative_ignore_section_role: bool = Field(default=True)  # env: NARRATIVE_IGNORE_SECTION_ROLE
    ...
    narrative_ignore_leaf_role: bool = Field(default=False)  # env: NARRATIVE_IGNORE_LEAF_ROLE
    # Narrative retrieval: skip answerability-pass hard gate; keep rerank-driven candidates.
    narrative_disable_answerability_gate: bool = Field(default=False)  # env: NARRATIVE_DISABLE_ANSWERABILITY_GATE
    # Narrative retrieval: enable post-rerank citationability + near-duplicate selector.
    narrative_post_rerank_selector_enabled: bool = Field(default=True)  # env: NARRATIVE_POST_RERANK_SELECTOR_ENABLED
    # Narrative retrieval: drop obvious non-citable narrative chunks after rerank.
    narrative_citationability_filter_enabled: bool = Field(default=True)  # env: NARRATIVE_CITATIONABILITY_FILTER_ENABLED
