...
### 4.1 章节边界的确定：两个"目录"来源，分开对待

**优先：PDF 内嵌书签（outline）**——排版软件写死的机器可读大纲，`fitz.open(source).get_toc()` 返回 `[级别, 标题, 起始物理页]` 三元组，页码权威零歧义：

- **切分边界**：章 N 页区间 = `[toc[N].page, toc[N+1].page - 1]`，末章 = `[..., 总页数]`。章节天然"另起一页"，页边界 = 章边界
- **级别归一化**：书里常是"章（一级）+节（二级）"，取最小级别为章级，只按章切，节留给分块器
- **切分前校验护栏**（书签本身不规范时防误切）：一级条目 ≥2；页码严格递增；同页多条目取首条；不满足任一 → 退化整本
- 已知局限：个别论文合集存在"章在页中间结束、下章同页开始"，页级切分把上章尾巴带进下章首页——电子书几乎不存在，v1 接受页级粒度并在 metadata 记 `split_note`

**兜底：排版目录页指引**（无内嵌书签的 PDF，即用户直觉里的"按目录页切"）：

1. 前 5 页文本（fitz 文本层，必要时 dots.ocr）识别目录页（"目录" + 多条 "章节名……页码" 行）
2. 正则解析（章标题, 书页码）序列（兼容点线/全角空格/中文数字）
3. **偏移校准**（关键）：书页码 ≠ PDF 物理页号（前面有封面/前言，罗马数字页）——取第一条章目在正文中搜索其标题实际落页，`偏移 = 实际物理页 − 目录页码`，套用到全部条目
4. 校验：偏移后页码递增且不越界；任一环节失败 → 退化整本入库并提示，**不承诺**任何 PDF 可拆

### 4.2 拆分与登记

- 页区间切物理 PDF：PyMuPDF `insert_pdf` 逐章生成子 PDF（与 09-19 课件手动拆章同法，纯本地零模型），章 N 页区间见 4.1
- **扉页/前言残页**（首章起始页之前的页）：非空则并入第一章（少一个 ghost 文档）
- 每章一个新 document_id（复用 allocate_document_id 的防撞预留），登记规则：
  - `book_id` = 书名主干（默认文件名主干，切分对话框可改）
  - `chapter_index` = 目录顺序号
  - `chapter_title` = TOC/目录页标题原文（如 "3 Efficient PEFT Design"）
  - `chapter_label` = `{book_id} · 第{N}章 · {chapter_title}` ← 列表/章节抽屉显示名；文件即书名+章节名的组合
  - 每章各自走六步管线（dots.ocr/fitz → 分块 → VLM 描述 → 向量化），**单章失败只标该章 failed**，其余章照常——部分成功天然可见
- **配套**：filters 面板书条目加"删除整本"（按 book_id 批量级联）——不然拆出的 12 章要手删 12 次；filters 面板效果：1 本书 N 章，书章筛选/章节抽屉/分页浏览全部原生可用（09-19 PEFT 三章聚合已验证此形态）
- **重拆同书**：同 book_id 再拆会并存两份章节（v1 不自动去重），配合"删除整本"先删后拆；文档写明
