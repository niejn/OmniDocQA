...
| v1.39 | DEV_PLAN §3.5 落实参考项目与测试素材: 参考项目=D:\mashibing\RAG_RAGAS\code(dots_ocr/Multimodal_RAG 实物, 对照查证用)+dots.ocr 官方仓库; 测试 PDF=主课件 84 页(拆 3 章测 --book 聚合, 内容自指)+英文论文 25 页(独立文档+跨语言), 页数/体积已核实在上限内 |
| v1.40 | 测试样例补全: 发现参考项目自带 Flink 第一章 PDF(14 页, 真实"章节 PDF"样例)+demo_pdf1(2 页, T1.2 bbox 首验最小样本); 书籍聚合组合=Flink 第一章+主课件拆 2 章共成《Apache Flink》一本书; gold set 主出题源=主课件 |
| v1.41 | 详设档案同步(DEV_PLAN §3.5 → PART2_DESIGN): §1 补参考项目实物路径; 新增 §17.2 测试素材与样例阶梯(四样例/聚合组合/demo_pdf1 bbox 首验); §10-4 验收标注样例来源 |
