...
> 日期：2026-09-16 · ~~状态：设计收口，待开工~~ → **✅ 2026-09-19 开发完成（MM-1/2/3/4 + T2.5 全部交付并真机验收；验收结果、注意事项与剩余待办见 `DEV_PROGRESS.md` 文末"2026-09-19 开发总结报告"节）** · 前置阅读：本文 + 【详设】§0（硬约束）
>
> 完成情况速览（2026-09-19）：
>
> | 里程碑 | 状态 | 证据要点 |
> |---|---|---|
> | MM-1 解析+入库（T1.1-T1.5） | ✅ | Flink 14 页 + PEFT 3 章聚合入库，dim=2048，幂等/降级/自然排序全过 |
> | MM-2 检索 API（T2.1-T2.4） | ✅ | 七端点 curl 验收；422/404 契约、filter 归一并集、删除级联全过 |
> | T2.5 评测门禁 | ✅ | 12 题校准 gold set；基线 HitRate@8=0.917 / MRR=0.861 / scope=0 |
> | MM-3 前端（T3.1-T3.3） | ✅ | `/documents` 页 + 7 代理路由，浏览器渲染与代理链路验证 |
> | MM-4 章节浏览 | ✅ | 分页端点 + 前端抽屉 |
> | 续作：资产 GC / 二期 MinIO / R2 链路 | ✅ | GC 0 孤儿；MinIO 迁移字节级 verified；reference 列 + 指标路由生效 |
>
> 单测 142 全绿（存量 69 + 新增 73）。遗留：dots.ocr vLLM 部署决策（决定后重入库获图片块）、R2 补标跑批、R3-R7。
